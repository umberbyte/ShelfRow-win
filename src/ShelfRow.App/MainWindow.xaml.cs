...
public sealed partial class MainWindow : Window
{
    ...
    public MainWindow()
    {
        ...
        // RatingControl does not clear when its currently selected star is clicked.
        // Observe handled pointer/tap events so the familiar "click again to clear"
    ...
    {
        if (width <= 0 || maxRating <= 0 || x < 0 || x > width)
            return 0;

        return Math.Clamp((int)Math.Ceiling(x / width * maxRating), 1, maxRating);
    }
