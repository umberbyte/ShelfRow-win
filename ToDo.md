...
- **実装済み (2026-09-22)**: 第1段階としてZIP/CBZ内の画像を安全な上限付きで列挙し、先頭候補を
  ローカルキャッシュへ原子的に抽出する経路を実装。画面表示時に最大2並列で生成し、版とサイズを保存する。

### [x] A-4. サムネイル配布の「提供側」が全て未配線
...
- **実装済み (2026-09-22)**: 生成成功時にNASへ原子的にコピーしてマニフェストを更新する。
  NAS不在時は端末ローカル状態を `PendingUpload` とし、次回サムネイル同期時に再送する。

---

## 優先度B — 静かに間違う

### [x] B-1. 旧 Stackroom のサムネイルを移行していない
...
- **実装済み (2026-09-22)**: XMLと同名の隣接フォルダを旧アセットルートとして検出し、新規・既存の
  どちらの書籍でも未移行の場合だけ非同期コピーする。

### [x] B-2. 外部キーが強制されておらず、インポートが順序事故に依存している
...
### [x] B-3. 本棚メンバーシップを iCloud へ送信していない
...
- **実CloudKit検証済み (2026-09-22)**: production で保留中だった CDMR 削除7件を `forceDelete` で送信し、
  サーバー応答後に保留キューが0件になったことを確認。さらに同期トークンなしの全件再取得で
  CDMR が19,217件から19,210件へ7件減り、削除通知も7件返ることを確認した。
...
- [x] **C-1. セキュリティ > 簡易ロック** — 起動時に全画面ロックを表示し、Enter/OKで照合して解除
- [x] **C-2. カスタマイズ > 独自フィールド名** — インスペクターのラベルと検索ボタンへ即時反映
- [x] **C-3. カスタマイズ > 独自タイプ名** — 種別フィルターボタンへ即時反映
- [x] **C-4. キーワード等価ルール** — `KeywordRules` 参照ゼロ。mac の `KeywordEquivalenceCodec`
      （検索語を等価語に展開する仕組み）に相当するものが無い
- [x] **C-5. カスタマイズ > リネーム書式** — Windows版には対象となるファイル追加・リネーム機能が
      ないため、誤認を招く設定UIを削除（保存済み設定値は互換性のためモデルに保持）
- [x] **C-6. 一般 > 外観・縮小表示・終了動作** — 保存だけされていた3設定をメイン画面へ即時反映。
      外観はシステム/ライト/ダーク、縮小表示は左右ペインと余白を75%、終了しない設定はタスクバーへ最小化
- [x] **C-7. iCloud同期スイッチと状態表示** — 常にオン・同期中を示すだけで未配線だった表示を削除し、
      Windows版の実際の挙動である操作時の双方向同期として説明を修正
- [x] **C-8. ヘルパー・キーワード設定の直接編集** — 一覧内で文字や対象項目を変更しても保存されなかったため、
      TextChanged / SelectionChanged を設定保存へ接続

---

## 優先度D — 未移植の画面

- [x] **D-1. スマートシェルフの編集** — 新規作成とコンテキストメニュー編集を実装。
      キーワード、種別、評価、未読、日付条件をmac互換JSONで保存
- [x] **D-2. 表紙の差し替え** — ZIP/CBZ内画像のスライダー式プレビュー、選択、保存、NAS再配布を実装
- [x] **D-3. スタンプ** — フォーカス中のインスペクター項目への挿入、一覧編集、JSON設定への永続化を実装
- [x] **D-4. 標準シェルフ操作** — 作成時の命名、名前変更、書籍の追加・解除を右クリックメニューへ実装
- [x] **D-5. 書籍コンテキストメニュー** — 開く、未読、レート、種類、シェルフ所属、ライブラリ削除を実装

---

## 追加監査で修正した同期不具合

- [x] **ローカル削除が iCloud へ送信されない** — Item / Shelf と関連 CDMR のレコード名を削除後も
      SQLite キューへ保持し、CloudKit の delete 成功後にだけキューから除去する。HTTP契約テストを追加
- [x] **「iCloudのデータを完全に削除」が確認表示だけで終了する** — `zones/modify` による Core Data
      カスタムゾーン削除を実装。文字入力による二重確認後に実行し、成功時だけ同期トークン消去とサインアウト
- [x] **共有 SQLite 接続を複数の非同期処理が同時使用する** — 表紙取得・編集保存・同期が同じ接続で
      衝突しないよう、リポジトリ操作をキャンセル可能な非同期ゲートで直列化。並行読み書き回帰テストを追加
- [x] **CloudKit 一括送信が既定で atomic** — 1件の競合で無関係な最大199件まで拒否されないよう
      `records/modify` を `atomic: false` にし、削除ペイロードも Apple の契約どおり recordName のみに修正
- [x] **受信 change token の確定が早すぎる** — 全ページ受信後の CDMR と Volume 参照解決が成功して
      からだけ最終トークンを保存し、中断時に関連付けを永久に取り逃がさないよう修正
- [x] **旧版と新版の表紙が同じバイト数だと更新されない** — 端末側に旧版の記録がある場合は
      ファイルサイズが一致しても必ず新版を取得。状態が無い旧キャッシュの初回採用だけサイズ照合を許可
- [x] **NAS取得失敗時に旧表紙まで消える／同じ表示要求で二度取得する** — 新版取得に失敗しても
      既存キャッシュを表示し、1回のロード内ではNASアクセスを一度だけに制限。同じ表紙の旧ロード完了が
      直後に始まった新ロードを処理中一覧から誤って外さないよう、同一ジョブを確認してから解除
- [x] **表紙生成失敗が永久キャッシュされる／NAS書込失敗で再送状態が失われる** — 生成ジョブは
      成否・キャンセルを問わず解除し、配布前にPendingUploadを保存して一時的なNAS障害後も再送可能に修正。
      配布目録の版管理ではItemを変更せず、表紙だけの更新をCloudKitへ送らないmac版の現行契約にも統一
- [x] **ウインドウをリサイズしても次回起動時に1280×800へ戻る** — 最後の通常表示サイズを設定へ保存し、
...
- [x] **E-3.** [`ApplePlistParser.cs:61`](src/ShelfRow.Importer/ApplePlistParser.cs#L61)
      日付の解析失敗時に `DateTime.UtcNow` を代入している。壊れたデータが
      「今日追加された本」として静かに紛れ込む。失敗は null にすべき
- [x] **E-4.** 同ファイル `:36` `List<byte>` に1バイトずつ追加している。数十MBの XML でメモリを倍使う
